				...
				if(reqParam !=NULL)
				{
					...
					else
					{
						...
						if((ccspStatus == 192) || (ccspStatus == 204) || (ccspStatus == 191))
						{
							...
							updateTmpList(gmp->entries[m].name_space, gmp->entries[m].etag, "pending", result, ccspStatus, 0, 1);
						...
						else
						{
							...
							WebcfgDebug("The result is %s\n",result);
							updateTmpList(gmp->entries[m].name_space, gmp->entries[m].etag, "failed", result, ccspStatus, 0, 0);
